...
/* ── Job Card ── */
function JobCard({ job, onStatus, onImg, isMobile }) {
  ...
  const actionArea = () => {
    ...
    if (job.status==="ACCEPTED")    return <ActionBtn label="Start Job" bg={C.amber}  color="#fff" onClick={()=>onStatus(job.id,"IN_PROGRESS")} icon={PlayIcon}/>;
      ...
      <div style={{
        ...
        {isMobile ? (
          <div style={{display:"flex",gap:8,width:"100%"}}>
            ...
            {job.status==="ACCEPTED"    && <button onClick={()=>onStatus(job.id,"IN_PROGRESS")} style={{flex:1,padding:"9px",borderRadius:9,border:"none",cursor:"pointer",background:C.amber,color:"#fff",fontSize:12,fontWeight:700}}>▶ Start Job</button>}
            ...
            {loading ? (
              ...
              shown.map((job,i) => (
                <div key={job.id} style={{ animation:`fadeUp ${.08+i*.04}s ease` }}>
                  <JobCard job={job} onStatus={setStatus} onImg={setModal} isMobile={mobile}/>
                </div>
              ))
            )}
          </div>
